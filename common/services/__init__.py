...
from common.services.ecowitt import Ecowitt, Category
from common.services.exceptions import ServiceError
from common.storage import to_file, Reading

LOGGER = logging.getLogger(__name__)


def _retrieve_live_readings(ecowitt: Ecowitt, device: tuple[str, str]):
...
def _retrieve_historical_readings(ecowitt: Ecowitt, device: tuple[str, str], last_updated: datetime):
...
def update_data(ecowitt: Ecowitt, settings: Settings):
    for device in ecowitt.list_devices():
        try:
            _retrieve_historical_readings(ecowitt=ecowitt, device=device, last_updated=settings.ecowitt.last_updated)
