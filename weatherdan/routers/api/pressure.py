...
from weatherdan.database.tables import PressureReading
...
def get_daily_readings(year: int | None = None, month: int | None = None) -> list[RangeReading]:
    with db_session:
        entries = sorted([x.to_model() for x in PressureReading.select()])
        if year:
...
def get_weekly_readings(
    ...
    with db_session:
        weekly = {}
        for entry in sorted(PressureReading.select(), key=lambda x: x.datestamp):
...
def add_reading(*, input: RangeReading) -> RangeReading:  # noqa: A002
    with db_session:
        if reading := PressureReading.get(datestamp=input.datestamp):
            reading.high = input.high
            reading.low = input.low
        else:
            reading = PressureReading(datestamp=input.datestamp, high=input.high, low=input.low)
...
def remove_reading(*, datestamp: date = Body(embed=True)) -> None:
    with db_session:
        reading = PressureReading.get(datestamp=datestamp)
        if not reading:
...
def refresh_readings(*, force: bool = False) -> None:
    ...
    if not force and settings.last_updated.pressure >= temp_time:
    ...
    with db_session:
        ...
        history_readings = ecowitt.get_history_readings(
            ...
            start_date=settings.last_updated.pressure,
        )
        for timestamp, value in history_readings.items():
            if reading := PressureReading.get(datestamp=timestamp.date()):
                if value > reading.high:
                    reading.high = value
                if value < reading.low:
                    reading.low = value
            else:
                reading = PressureReading(datestamp=timestamp.date(), high=value, low=value)
        # endregion
        # region Live reading
        live_reading = ecowitt.get_live_reading(device=device.mac, category=Category.PRESSURE)
        if live_reading:
            if reading := PressureReading.get(datestamp=live_reading.time.date()):
        ...
        # endregion
    settings.last_updated.pressure = datetime.now()  # noqa: DTZ005
    settings.save()
