...
def historical(
    ...
    def year_list() -> list[int]:
        with db_session:
            return sorted({x.datestamp.year for x in SolarReading.select()})

    def month_list(year: int) -> list[int]:
        with db_session:
            return sorted(
                {x.datestamp.month for x in SolarReading.select() if x.datestamp.year == year},
            )

    return templates.TemplateResponse(
        "solar/historical.html.jinja",
        {
            "request": request,
            "count": count,
            "year_list": year_list(),
