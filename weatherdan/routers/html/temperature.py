...
def current(
    ...
    return templates.TemplateResponse(
        "temperature/current.html.jinja",
...
def historical(
    ...
    def year_list() -> list[int]:
        with db_session:
            return sorted({x.datestamp.year for x in TemperatureReading.select()})
    ...
    def month_list(year: int) -> list[int]:
        with db_session:
            return sorted(
                {
                    x.datestamp.month
                    for x in TemperatureReading.select()
                    if x.datestamp.year == year
                },
            )

    return templates.TemplateResponse(
        "temperature/historical.html.jinja",
        {
            "request": request,
            "count": count,
            "year_list": year_list(),
