...
def historical(
    ...
    def year_list() -> list[int]:
        with db_session:
            return sorted({x.datestamp.year for x in WindReading.select()})

    def month_list(year: int) -> list[int]:
        with db_session:
            return sorted(
                {x.datestamp.month for x in WindReading.select() if x.datestamp.year == year},
            )

    return templates.TemplateResponse(
        "wind/historical.html.jinja",
        {
            "request": request,
            "count": count,
            "year_list": year_list(),
