...
class Ecowitt:
    ...
    def __init__(self: Self, timeout: float = 30.0):
        self.headers = {
    ...
    def list_devices(self: Self) -> list[tuple[str, str]]:
    ...
    def list_device_history(
        ...
        results = self._get_request(
            ...
            params={
                ...
                "end_date": datetime.now(tz=UTC).astimezone().isoformat(),
        ...
        if not results:
            return {}
        return {
            datetime.fromtimestamp(int(k), tz=UTC).astimezone(): v
            for k, v in results[category.group_1][category.group_2]["list"].items()
        }

    def get_device_reading(self: Self, mac: str, category: Category) -> tuple[datetime, str]:
        results = self._get_request(
